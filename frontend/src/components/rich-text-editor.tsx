...
// 글자 크기 입력 칸에서 허용할 최소/최대 크기(px)입니다.
...
export default function RichTextEditor({ value, onChange }: Props) {
  ...
  // 커서를 옮기기만 해도(내용은 그대로여도) 툴바가 실제 커서 위치의 서식을 정확히 보여주도록
  // 화면을 강제로 한 번 더 그리게 만드는 용도입니다. 값 자체는 쓰이지 않고, 숫자를 바꿔서
  // 리액트가 "뭔가 바뀌었다"고 인식하게만 하면 됩니다.
  ...
  const editor = useEditor({
    ...
    // 커서 위치가 바뀌거나(글자를 선택하거나 커서만 옮겨도) 화면을 다시 그려서,
    // 툴바에 보이는 굵게/글꼴/글자크기/색상 등이 항상 "지금 커서가 있는 곳"의 실제 서식을 보여주게 합니다.
    onSelectionUpdate: () => {
      forceRerender((n) => n + 1);
    },
    onTransaction: () => {
  ...
  function handleFontSizeChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    const clamped = Math.min(MAX_FONT_SIZE, Math.max(MIN_FONT_SIZE, raw));
  ...
  const currentFontSizeRaw = editor.getAttributes("textStyle").fontSize as string | undefined;
  const currentFontSize = currentFontSizeRaw ? parseInt(currentFontSizeRaw, 10) : DEFAULT_FONT_SIZE;
    ...
    <div>
      <div className="flex flex-wrap items-center gap-1 rounded-t-lg border border-neutral-200 bg-neutral-50 px-2 py-1.5">
        ...
        <input
          ...
          className="w-14 cursor-text rounded border border-neutral-200 bg-white px-1.5 py-1.5 text-xs"
        />
        ...
        <div className="flex items-center gap-1 rounded border border-neutral-200 bg-white px-1.5 py-1">
          ...
          {/* 직접 선택 - 원하는 색상을 자유롭게 고를 수 있습니다. */}
          <input
            type="color"
            value={currentColor}
            onChange={(e) => editor.chain().focus().setColor(e.target.value).run()}
            title={t("editor.colorCustom")}
            className="h-5 w-6 cursor-pointer rounded border border-neutral-200 bg-white p-0"
          />
          <button
            type="button"
