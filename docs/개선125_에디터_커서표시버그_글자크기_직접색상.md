# 개선125 - 에디터 커서 위치 표시 버그 수정 + 글자 크기(숫자) + 색상 직접 선택 추가

## 이번 가이드에서 고치는 것

1. **버그**: 글자 A에 폰트A, 글자 B에 폰트B처럼 서로 다른 서식을 준 뒤 커서를 이리저리 움직이면, 툴바(글꼴/색상/굵게 등 표시)가 실제 커서 위치의 서식이 아니라 "가장 최근에 눌렀던 버튼"의 상태를 계속 보여주는 문제였습니다.
   - **원인**: 이 에디터가 쓰고 있는 Tiptap이라는 라이브러리는 최신 버전(v3)부터, 화면 성능을 위해 "커서만 움직였을 때는 화면을 자동으로 다시 그리지 않도록" 기본값이 바뀌었습니다. 그래서 실제 문서 내용은 정확한데(즉, "b"만 폰트B로 바뀐 게 맞습니다), 화면에 보여주는 툴바 상태만 갱신이 안 되고 있었던 것입니다. 커서를 옮겨도 "다시 그리기"가 일어나게 해주면 해결됩니다.
2. **기능 추가 - 글자 크기(숫자로 조절)**: 정해진 옵션이 아니라 숫자(px)로 직접 글자 크기를 입력할 수 있는 칸을 추가합니다.
3. **기능 추가 - 색상 직접 선택**: 미리 정해둔 7가지 색상 외에도, 색상 선택기를 통해 원하는 색을 자유롭게 고를 수 있도록 추가합니다.

---

## 1단계: `rich-text-editor.tsx` 전체 교체하기

`frontend/src/components/rich-text-editor.tsx` 파일을 엽니다. 파일 안의 내용을 전부 지우고, 아래 내용으로 통째로 바꿔주세요.
...
**바뀐 부분 요약**:
- `import { TextStyle } from "@tiptap/extension-text-style";` → `import { TextStyle, FontSize } from "@tiptap/extension-text-style";` (글자 크기 기능도 같은 패키지 안에 이미 포함되어 있어서, 새로 설치할 필요 없이 이름만 하나 더 꺼내 쓰면 됩니다.)
- `extensions` 배열에 `FontSize` 추가
- `useEditor` 설정에 `onSelectionUpdate`, `onTransaction` 콜백 추가 → 커서만 움직여도 화면이 다시 그려지도록(버그 수정)
- 툴바에 글자 크기 숫자 입력 칸 추가 (`MIN_FONT_SIZE`~`MAX_FONT_SIZE` px 사이로 제한)
- 글자 색상 영역에 `<input type="color">` 색상 선택기 추가 (원하는 색을 자유롭게 선택)
...
2. **버그 확인**: 글 안에 문장 a, 문장 b를 씁니다. b만 선택해서 폰트를 다른 것으로 바꿉니다. 그 다음 커서만 클릭으로 a → b → a 순서로 옮겨보면서, 툴바의 글꼴 표시가 각 위치에 맞게 정확히 바뀌는지 확인합니다. (같은 방식으로 색상, 굵게, 정렬 등도 커서 위치에 따라 정확히 표시되는지 확인해보세요.)
3. **글자 크기 확인**: 글자를 선택하고 새로 생긴 숫자 입력 칸에 원하는 크기(예: 24)를 입력하면 글자 크기가 바뀌는지 확인합니다. 커서를 다른 크기의 글자로 옮기면 숫자도 그에 맞게 바뀌는지 확인합니다.
4. **색상 직접 선택 확인**: 색상 영역의 네모난 색상 선택기를 눌러 원하는 색을 골라보고, 글자에 적용되는지 확인합니다.
...
git add .
git commit -m "에디터 커서 위치 표시 버그 수정, 글자 크기(숫자)/색상 직접 선택 기능 추가"
git push
```
...
- [ ] `rich-text-editor.tsx`를 전체 교체했다
- [ ] `dictionary.ts`에 `editor.fontSize`, `editor.colorCustom`을 한국어/영어 모두 추가했다
- [ ] 커서를 옮길 때마다 툴바 표시(글꼴/색상/굵게 등)가 정확하게 바뀐다
- [ ] 글자 크기를 숫자로 직접 입력해서 바꿀 수 있다
- [ ] 색상을 직접 선택해서 적용할 수 있다
- [ ] GitHub에 커밋하고 푸시했다
